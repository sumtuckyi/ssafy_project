<template>
	<div class="container">
		<div class="wrapper">
			<button @click="goBack" id="back-btn">뒤로가기</button>
			<h1>상품 안내</h1>
			<div class="box">
				<p class="join-way">
				...
				<div class="content">
					...
					<p><strong>우대조건 :</strong>우대조건 : {{ data.spcl_cnd }}</p>
...
const max_limit = computed(() => {
	return data.value.max_limit !== -1 ? Number(data.value.max_limit).toLocaleString()  : '없음'
})
...
li {
	...
	line-height: 2;
	/* border-bottom: 0.1px rgba(92, 85, 85, 0.993) dotted ; */
}
.opt-item {
	display: flex;
}
